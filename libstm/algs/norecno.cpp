...
/**
 *  NOrec Non-Opaque Implementation
 *
...
namespace {
  ...
  void
  NOrecno_Generic<CM>::initialize(int id, const char* name)
  {
      // set the name
      stm::stms[id].name = name;

      // set the pointers
      stm::stms[id].begin     = NOrecno_Generic<CM>::begin;
      stm::stms[id].commit    = NOrecno_Generic<CM>::commit_ro;
      stm::stms[id].read      = NOrecno_Generic<CM>::read_ro;
      stm::stms[id].write     = NOrecno_Generic<CM>::write_ro;
      stm::stms[id].local_write = NOrecno_Generic<CM>::local_write;
      stm::stms[id].irrevoc   = irrevoc;
      stm::stms[id].switcher  = onSwitchTo;
      stm::stms[id].privatization_safe = true;
      stm::stms[id].rollback  = NOrecno_Generic<CM>::rollback;
  }

  template <class CM>
  bool
  NOrecno_Generic<CM>::begin(TxThread* tx)
  {
  ...
  template <class CM>
  void
  NOrecno_Generic<CM>::commit(STM_COMMIT_SIG(tx,upper_stack_bound))
  {
  ...
  template <class CM>
  void
  NOrecno_Generic<CM>::commit_ro(STM_COMMIT_SIG(tx,))
  {
	  tx->insert_time = tx->insert_time + getElapsedTime() - tx->start;
	  if(tx->start_time != timestamp.val)
		  if ((tx->start_time = validate(tx)) == VALIDATION_FAILED)
  ...
  template <class CM>
  void
  NOrecno_Generic<CM>::commit_rw(STM_COMMIT_SIG(tx,upper_stack_bound))
  {
...
tx->reads = (tx->reads + tx->vlist.size())/2;
  ...
  template <class CM>
  void*
  NOrecno_Generic<CM>::read_ro(STM_READ_SIG(tx,addr,mask))
  {
...
//tx->reads++;
  ...
  template <class CM>
  void*
  NOrecno_Generic<CM>::read_rw(STM_READ_SIG(tx,addr,mask))
  {
  ...
  template <class CM>
  void
  NOrecno_Generic<CM>::write_ro(STM_WRITE_SIG(tx,addr,val,mask))
  {
  ...
  template <class CM>
    void
    NOrecno_Generic<CM>::write_rw(STM_WRITE_SIG(tx,addr,val,mask))
    {
  ...
  template <class CM>
    void
    NOrecno_Generic<CM>::local_write(STM_WRITE_SIG(tx,addr,val,mask))
    {
        *addr = val;
    }
  ...
  template <class CM>
  stm::scope_t*
  NOrecno_Generic<CM>::rollback(STM_ROLLBACK_SIG(tx, upper_stack_bound, except, len))
  {
      stm::PreRollback(tx);
...
#define FOREACH_NORECNO(MACRO)                    \
    MACRO(NOrecno, HyperAggressiveCM)             \
    MACRO(NOrecnoHour, HourglassCM)               \
    MACRO(NOrecnoBackoff, BackoffCM)              \
    MACRO(NOrecnoHB, HourglassBackoffCM)

#define INIT_NORECNO(ID, CM)                      \
    template <>                                 \
    void initTM<ID>() {                         \
        NOrecno_Generic<CM>::initialize(ID, #ID);     \
    }

namespace stm {
  FOREACH_NORECNO(INIT_NORECNO)
}

#undef FOREACH_NORECNO
#undef INIT_NORECNO
