 ...
 *
 * The ChunkedLog is now ready for helper threads. A worker thread can
 * insert values into the log while a helper thread reads/iterates it
 */
...
#define CHUNKED_LOG 1

#ifdef CHUNKED_LOG

namespace wlpdstm {

	template<typename T, int CHUNK_LENGTH = 256, bool INIT = false>
	class Log {
		private:
			struct LogChunk {
				LogChunk() : elements(static_cast<T*>(malloc(sizeof(T)*CHUNK_LENGTH))),
						next(NULL), prev(NULL) { }
		...
		public:
			class iterator {
				public:
					iterator(LogChunk *c = NULL, unsigned el = 0, unsigned lel = 0)
						: chunk(c), element(el), currentElement(el), lastElement(lel) { }
		...
		public:
			Log() : head(new LogChunk()), lastChunk(head),
				nextElement(0), changedSize(0), size(0) { }
...
namespace wlpdstm {
	...
	inline typename Log<T, CHUNK_LENGTH, INIT>::iterator Log<T, CHUNK_LENGTH, INIT>::end() const {
		...
		return iterator(lastChunk, size, size);
	}
	...
	template<typename T, int CHUNK_LENGTH, bool INIT>
	inline bool Log<T, CHUNK_LENGTH, INIT>::hasChanged() {
		bool ret = (size != changedSize);
		changedSize = size;
